...
class CropComparison:
    ...
    """
    def __init__(self, reference_image, method_images, crops: List[image.Cropbox], spp_list,
        ...
        self._ref_grid[0, 0].image = self.tonemap(method_images[0])
        for crop in crops:
    ...
    def tonemap(self, img):
        return figuregen.JPEG(image.lin_to_srgb(img), quality=100)
    ...
    def compute_error(self, reference_image, method_image) -> Tuple[str, List[float]]:
        return image.relative_mse_outlier_rejection(method_image, reference_image, 0.000001, 0.1)
        return image.mse(method_image, reference_image)
    ...
    def error_string(self, index: int, errors: List[float]):
        ...
        if self.use_latex and index == np.argmin(errors):
            return f"$\\mathbf{{{errors[index]:.4f} ({errors[index]/errors[0]:.1f}\\times)}}$"
        elif self.use_latex:
            return f"${errors[index]:.3f} ({errors[index]/errors[0]:.1f}\\times)$"
        else:
            return f"{errors[index]:.4f}, {self.spp_list[index]}"
...
class FullSizeWithCrops:
    ...
    def error_string(self, index: int, errors: List[float]):
        ...
        """
        if self.use_latex and index == np.argmin(errors):
            return f"$\\mathbf{{{errors[index]:.3f} ({errors[index]/errors[0]:.2f}\\times)}}$"
        elif self.use_latex:
            return f"${errors[index]:.3f} ({errors[index]/errors[0]:.2f}\\times)$"
        else:
            return f"{errors[index]:.3f} ({errors[index]/errors[0]:.2f}x)"
