...
bool SamplingIntegrator::render(Scene *scene,
    ...
    while(true){
        ParallelProcess::EStatus status = proc->getReturnStatus();
        if (status != ParallelProcess::EUnknown) {
            break;
        }
        if (m_timeout > 0) {
            float elapsedMs = (float)renderingTimer->getMilliseconds();
            if (elapsedMs >= m_timeout * 1000.f) {
                Scheduler::getInstance()->cancel(proc);
                Log(EWarn, "SamplingIntegrator: rendering canceled due to timeout (%.2f s).",
                           (double)(elapsedMs / 1000.0f));
                break;
            }
        }
    }
...
MonteCarloIntegrator::MonteCarloIntegrator(const Properties &props) : SamplingIntegrator(props) {
    ...
    m_timeout = props.getInteger("timeout", 0);
