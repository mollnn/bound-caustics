...
public:
	...
	Interval1D(double _min, double _max) :vMin(_min), vMax(_max){}
	Interval1D(double _min) :vMin(_min), vMax(_min){}
	...
	inline Interval1D operator-(const double &b) const {
	...
	inline Interval1D operator*(double b) const {
	...
	{
		double vMin_2 = vMin * vMin;
		double vMax_2 = vMax * vMax;
		double maxSquare = max(vMin_2, vMax_2);
	...
	{
		return Interval1D(std::max(vMin, 0.0), std::max(vMax, 0.0));
	}
...
inline Interval1D minmax(const Interval1D &a, const  double&b)
{
	return Interval1D(min(a.vMin, b), max(a.vMax, b));
}

inline Interval1D minus1D(const double &a, const Interval1D &b)
{
...
public:
	...
	inline Interval3D operator*(double b) const {
		Interval1D result[3];
		for (int i = 0; i < 3; i++)
		{
			result[i] = value[i] * b;
	...
	inline Interval3D normalized()
	{
		...
		if (lengthSquare.vMin < 0) return Interval3D(Vec3(-1, -1, -1), Vec3(1, 1, 1));
		...
		for (int i = 0; i < 3; i++)
		{
			...
			result[i].vMin = std::max(result[i].vMin, -1.0);
			result[i].vMax = std::min(result[i].vMax, 1.0);
		}
	...
	inline bool smallerThan( double threshold){
		for (int i = 0; i < 3; i++)
		{
	...
	{
		//surface area
		double area = value[0].length() * value[1].length() +
	...
	inline double volumeSphere()
	{
		double maxDist = -1;
		for (int i = 0; i < 3; i++)
		{
			double temp = value[i].vMax - value[i].vMin;
	...
	inline Vec3 center(double &radius)
	{
		Vec3 result;
		double r = 0.0f;
		for (int i = 0; i < 3; i++)
		{
			...
			double temp = value[i].vMin - result[i];
			r += temp*temp;
		}
		radius = sqrtf(r);
		return result;
...
public:
	...
	inline Interval2D operator*(double b) const {
		Interval1D result[2];
		for (int i = 0; i < 2; i++)
		{
			result[i] = value[i] * b;
	...
	{
		Interval1D lengthSquare = squaredNorm();
		if (lengthSquare.vMin < 0) return Interval2D(Vec2(-1, -1), Vec2(1, 1));
...
public:
	...
	inline Interval4D operator*(double b) const {
		Interval1D result[4];
		for (int i = 0; i < 4; i++)
		{
			result[i] = value[i] * b;
	...
	{
		Interval1D lengthSquare = squaredNorm();
		if (lengthSquare.vMin < 0) return Interval4D(Vec4(-1, -1,-1,-1), Vec4(1, 1,1,1));
