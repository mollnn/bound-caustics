...
public:
	PathVert(){}
	PathVert(const Vec3 &_pos, const Vec3 &_nor, const Vec3 &_intensity) :
		m_pos(_pos), m_n(_nor), m_intensity(_intensity){
			m_bsdf = NULL;
			m_N = m_n.normalized();
			m_N_G = m_N;
		}
		...
		m_bsdf(_bsdf), m_ishape(_ishape), m_itriangle(_itriangle)
	{
		...
		Eigen::Vector4d P(m_pos[0], m_pos[1], m_pos[2], 1);
	...
	PathVert(const Vec3 &_pos, const Vec3 &_nor,
		const Vec3 &vert1, const Vec3 &vert2, const Vec3 &vert3,
		const Vec3 &vN1, const Vec3 &vN2, const Vec3 &vN3,
		const Vec3 &fN, MyBSDF *_bsdf
		) : m_pos(_pos), m_n(_nor), Nalpha(vN1), Nbeta(vN2), Ngamma(vN3),
		m_bsdf(_bsdf)
	{
		m_intensity = Vec3(1,1,1);
		m_N = m_n.normalized();
...
{
public:
	Path(vector<PathVert> &path, double pdf)
	{
		vertices.swap(path);
		m_pdf = pdf;
	}
	Path(vector<PathVert> &path, vector<int> &tid, double pdf)
	{
		vertices.swap(path);
	...
	Vec3 computeContribution() const
	{
		...
		for (int i = 1; i < pathLength - 1; i++)
		{
			...
			Vec3 p2d = (vertices[i - 1].m_pos - p.m_pos).normalized();
			Vec3 fn = p.m_N_G;
			Vec3 sn = p.m_N;
			// if (p2c.dot(fn) * p2d.dot(fn) > 0)
			// {
			// 	return Vec3(0.0, 0.0, 0.0);
			// }
			// if (p2c.dot(sn) * p2d.dot(sn) > 0)
			// {
			// 	return Vec3(0.0, 0.0, 0.0);
			// }
