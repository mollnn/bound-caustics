...
std::vector<std::tuple<double, double, double, double>> solveImpl(
    ...
    double vmax)
{
    // ! WARN: pi = pi0 + u pi1 + v pi2    (pi1/2 is already delta)
    ...
    // todo: add if else according to bounce
    // * bounce = 1, R, degree = 4
    if (chain_type == 1) {
        BVP3 d0 = x1 - xD;
        BVP3 d1 = xL - x1;
        ...
        // BVP d0_dot_t1hat1 = d0.dot(t1hat1);
        // BVP d1_dot_t1hat1 = d1.dot(t1hat1);
        BVP d0_dot_t1hat2 = d0.dot(t1hat2);
        BVP d1_dot_t1hat2 = d1.dot(t1hat2);
        Czy = d0_dot_n1hat * d1_dot_t1hat2 + d0_dot_t1hat2 * d1_dot_n1hat;
        // Cxz = d0_dot_n1hat * d1_dot_t1hat2 + d0_dot_t1hat2 * d1_dot_n1hat;
        // Czy = Czy + Cxz;

        BVP3 s = xL - xD;
        BVP3 cop = (d0.cross(s)).cross(n1hat.cross(s));
        Cxz = cop.bvp[0] + cop.bvp[1] + cop.bvp[2]; // projection to only one axis may fail in some cases
        ...
        //     for (int j = 0; j <= NU; j ++) {
        //         double u = j * 1.0 / NU;
        //         double u2hat_val = u2hat.evaluateAtU(u).evaluateAtU(v);
        //         double v2hat_val = v2hat.evaluateAtU(u).evaluateAtU(v);
        //         double kappa2_val = kappa2.evaluateAtU(u).evaluateAtU(v);
        //         double u2_val = u2hat_val / kappa2_val;
        //         double v2_val = v2hat_val / kappa2_val;
        //         if ((u2_val >= 0 - eps && u2_val <= 1 + eps && v2_val >= 0 - eps && v2_val <= 1 + eps && u2_val + v2_val >= 0 - eps && u2_val + v2_val <= 1 + eps)) {
        //             flag = true;
        //             break;
        //         }
        //     }
        //     if (flag) {
        //         valid_v_min = min(valid_v_min, v);
        //         valid_v_max = max(valid_v_max, v);
        //     }
        // }
        // valid_v_min = max(0.0, valid_v_min - 1.0 / NV);
        // valid_v_max = min(1.0, valid_v_max + 1.0 / NV);
        ...
        BVP Czy1 = omega1hat_dot_n2hat * (d2hat.dot(t21hat)) + d2hat_dot_n2hat * (omega1hat.dot(t21hat)); // todo: change name
        ...
        if (pieceIdx == 6) {
            ...
            // valid_v_min = vminlist[pieceIdx] - (vmaxlist[pieceIdx] - vminlist[pieceIdx]) * 0.1;
            // valid_v_max = vmaxlist[pieceIdx] + (vmaxlist[pieceIdx] - vminlist[pieceIdx]) * 0.1;
    ...
    // Czy.resize(local_cutoff_matrix, local_cutoff_matrix + 5); // todo: make this interface clearer
    // Cxz.resize(local_cutoff_matrix, local_cutoff_matrix + 5);

    // Czy.printCoefficientsMatrix();
    // Cxz.printCoefficientsMatrix();

    Czy.resize(local_cutoff_matrix, local_cutoff_matrix); // todo: make this interface clearer
...
#ifdef RESULTANT_TRACE
    rss << Czy.toString() << endl;
    rss << Cxz.toString() << endl;
#endif

    auto ___time_end = std::chrono::high_resolution_clock::now();
    ___perf_time_coefficient += std::chrono::duration_cast<std::chrono::nanoseconds>(___time_end - ___time_begin).count() * 1e-3;
...
#ifdef RESULTANT_TRACE
        rss << poly_bezoutMatrix.first[0].first.toString() << endl;
...
std::vector<std::tuple<double, double, double, double>> solve(
    ...
    for (int dl = 0; dl < (chain_type == 11 ? 1 : 1); dl++) {
