...
std::vector<pair<UnivariatePolynomial, pair<double, double>>> determinant(const UnivariatePolynomialMatrix &matrix)
{
    ...
    else if (n == 5)
    {
        ...
        UnivariatePolynomial temp = (a[0] * b[1] - a[1] * b[0]) * (c[2] * (d[3] * e[4] - d[4] * e[3]) - c[3] * (d[2] * e[4] - d[4] * e[2]) + c[4] * (d[2] * e[3] - d[3] * e[2]))
                                  - (a[0] * b[2] - a[2] * b[0]) * (c[1] * (d[3] * e[4] - d[4] * e[3]) - c[3] * (d[1] * e[4] - d[4] * e[1]) + c[4] * (d[1] * e[3] - d[3] * e[1])) 
                                  + (a[0] * b[3] - a[3] * b[0]) * (c[1] * (d[2] * e[4] - d[4] * e[2]) - c[2] * (d[1] * e[4] - d[4] * e[1]) + c[4] * (d[1] * e[2] - d[2] * e[1])) 
                                  - (a[0] * b[4] - a[4] * b[0]) * (c[1] * (d[2] * e[3] - d[3] * e[2]) - c[2] * (d[1] * e[3] - d[3] * e[1]) + c[3] * (d[1] * e[2] - d[2] * e[1])) 
                                  + (a[1] * b[2] - a[2] * b[1]) * (c[0] * (d[3] * e[4] - d[4] * e[3]) - c[3] * (d[0] * e[4] - d[4] * e[0]) + c[4] * (d[0] * e[3] - d[3] * e[0])) 
                                  - (a[1] * b[3] - a[3] * b[1]) * (c[0] * (d[2] * e[4] - d[4] * e[2]) - c[2] * (d[0] * e[4] - d[4] * e[0]) + c[4] * (d[0] * e[2] - d[2] * e[0])) 
                                  + (a[1] * b[4] - a[4] * b[1]) * (c[0] * (d[2] * e[3] - d[3] * e[2]) - c[2] * (d[0] * e[3] - d[3] * e[0]) + c[3] * (d[0] * e[2] - d[2] * e[0])) 
                                  + (a[2] * b[3] - a[3] * b[2]) * (c[0] * (d[1] * e[4] - d[4] * e[1]) - c[1] * (d[0] * e[4] - d[4] * e[0]) + c[4] * (d[0] * e[1] - d[1] * e[0])) 
                                  - (a[2] * b[4] - a[4] * b[2]) * (c[0] * (d[1] * e[3] - d[3] * e[1]) - c[1] * (d[0] * e[3] - d[3] * e[0]) + c[3] * (d[0] * e[1] - d[1] * e[0])) 
                                  + (a[3] * b[4] - a[4] * b[3]) * (c[0] * (d[1] * e[2] - d[2] * e[1]) - c[1] * (d[0] * e[2] - d[2] * e[0]) + c[2] * (d[0] * e[1] - d[1] * e[0]));
        res.push_back(make_pair(temp, make_pair(0.0, 1.0)));
        return res;
    }
...
//     time_total += std::chrono::duration_cast<std::chrono::nanoseconds>(time_1 - time_begin).count() * 1e-3;
//     count_total += 1;
...
//     }
    ...
    // if (rand() % 10000 == 0) cout << "det " << det <<endl;
...
std::vector<std::vector<double>> evaluateMatrixAtU(const UnivariatePolynomialMatrix &matrix, double x)
{
    ...
    // if (rand() % 10000 == 0) {
    //     std::cout << "eval time " << time_total / count_total << std::endl;
    // }
