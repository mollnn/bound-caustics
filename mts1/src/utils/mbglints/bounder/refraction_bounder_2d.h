...
class RefractionBounder2d : public Bounder
{
public:
    RefractionBounder2d(const int k, const int id) : Bounder(k, id) {}

    inline void get_rational_single_reflection(const TriangleSequence &tseq, const BounderDomain &u1_domain, BVP<7> &u2p, BVP<7> &v2p, BVP<6> &k2, BVP<3> &C, BVP<3> &s0, BVP<7> &s1, BVP<3> &s2, bool approx_deriv = false)
    {
#ifdef BOUNDER_TIMER
        ...
        BVP<5> beta = n1.dot(n1) * d0.dot(d0) - (n1.dot(n1) * d0.dot(d0) - n1.dot(d0) * n1.dot(d0)) * eta * eta;
        BoundingVal I_beta = beta.bound();
        I_beta.m = std::max(I_beta.m, approx_deriv ? beta_MIN * 0.5 : 0);
        I_beta.M = std::max(I_beta.M, beta_MIN * 0.99);
        double mid = I_beta.m;
        double a1n = 0.f, a0n = 0.f;

        if (approx_deriv)
        {
            auto [a1n_t, a0n_t, delta_xin, delta_xi1n] = compute_easy_approx_for_sqrt(I_beta.m, I_beta.M);
            a1n = a1n_t;
            a0n = a0n_t;
        }
        else
        {
            auto [a1n_t, a0n_t, delta_xin] = compute_line_approx_for_sqrt(I_beta.m, I_beta.M);
            a1n = a1n_t;
            a0n = a0n_t;
        }
        ...
        BVP<1> a1(a1n);
        BVP<1> a0(a0n);
        BVP<5> sqrt_beta = a1 * beta + a0;

        BVP3<6> d1 = (d0 * (n1.dot(n1)) - n1 * n1.dot(d0)) * eta - n1 * sqrt_beta;
        ...
        // printPolyMatrix(sqrt_beta.coeffs);
        // std::cout<<"--------------------"<<std::endl;
        // printPolyMatrix(d1.x.coeffs);
    ...
    inline std::tuple<bool, bool, bool, bool, bool, double, double, BounderDomain>
    positional_check(const BounderDomain &u1_domain, BVP<7> &u2p, BVP<7> &v2p, BVP<6> &k2, BVP<3> &C, BVP<3> &s0, BVP<7> &s1, BVP<3> &s2)
    {
        BoundingVal u2_b = u2p.fbound(k2);

        // printPolyMatrix(u2p.coeffs);
        // std::cout<<"-------------"<<std::endl;
        // printPolyMatrix(k2.coeffs);
        // std::cout<<u2_b.m<<" "<<u2_b.M<<std::endl;
    ...
    inline void irradiance_explicit(const BounderDomain &u1_domain, BVP<7> &u2p, BVP<7> &v2p, BVP<6> &k2, BVP<3> &C, BVP<21> &Jp, BVP<25> &Jq)
    {
        BVP<7> u2p_du = u2p.du();
        BVP<7> u2p_dv = u2p.dv();
        BVP<7> v2p_du = v2p.du();
        ...
        BVP<12> u2u1p = u2p_du * k2 - k2_du * u2p;
        BVP<12> u2v1p = u2p_dv * k2 - k2_dv * u2p;
        BVP<12> v2u1p = v2p_du * k2 - k2_du * v2p;
        BVP<12> v2v1p = v2p_dv * k2 - k2_dv * v2p;

        BVP<11> denom = k2 * k2;

        BVP<23> fq = u2u1p * v2v1p;
        BVP<23> fq1 = u2v1p * v2u1p;
        fq = fq - fq1;
    ...
    void bound_bfs(const TriangleSequence &tseq, const BounderDomain &bfs_node) override
    {
        ...
        if (ans >= 0)
        {
            get_rational_single_reflection(tseq, bfs_node, u2p, v2p, k2, C, s0, s1, s2, true);
