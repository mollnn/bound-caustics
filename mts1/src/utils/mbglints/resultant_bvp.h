...
struct BVP3
{
    ...
    BVP3 operator * (const BVP3& rhs) const { // element-wise product
        return {bvp[0]*rhs.bvp[0], bvp[1]*rhs.bvp[1], bvp[2]*rhs.bvp[2]};
    }
    ...
    BivariatePolynomial dot(const BVP3& rhs) const { // dot product
        return bvp[0] * rhs.bvp[0] + bvp[1] * rhs.bvp[1] + bvp[2] * rhs.bvp[2];
    }

    BVP3 cross(const BVP3& rhs) const { // cross product
