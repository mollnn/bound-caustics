...
bool intersect_line_triangle(Vec3 q1, Vec3 q2, Vec3 p1, Vec3 p2, Vec3 p3, Vec3 &its)
{
	...
	if (s1 != s2)
	{
		...
		if (s3 == s4 && s4 == s5)
		{
			...
			double t = (p1 - q1).dot(n) / (q2 - q1).dot(n);
			its = q1 + t * (q2 - q1);
			return true;
		}
	}
...
public:
	...
	double dotvv(const Vec3 &v1, const Vec3 &v2)
	{
		return v1[0] * v2[0] + v1[1] * v2[1] + v1[2] * v2[2];
	}

	// create a shape for the input height field.
	MyShape(const Vec3 &center, string inputHFName, double hfLength,
			const Vec3 &normal, const Shape *shape,
			double maxHeight, MyBSDF *bsdf1, MyBSDF *bsdf2, const Vec3 &camPos, const Scene *scene)
	{
		m_center = center;
		m_length = hfLength;
		...
		const double heightScale = maxHeight / maxH;
		double pixelScale = m_length / m_width;
	...
	{
		AxisTriangle() {}
		AxisTriangle(double _value, int _triangle)
		{
			value = _value;
			triangle = _triangle;
		}
	...
	TreeNode *buildTreeForMeshMedian(const AABB &aabb, const std::vector<int> &triangles)
	{
		if (triangles.size() <= 4)
		{
			...
			for (int i = 0; i < triangles.size(); i++)
			{
				...
				{
					const Vec3 &inor = m_vertexNormal[idx[k]];
					norCone = max(norCone, 1.0 * acos(inor.dot(norm)));
				}
			}
			node->posBox = dataBound;
			node->norBox = normBound;
