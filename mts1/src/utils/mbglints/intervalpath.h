...
public:
	...
	Interval3D computeHalfV(const Interval3D &in, const Interval3D &out, 
		int type, double etaIn, double etaOut) const
	{
		switch (type) {
		case -1:
	...
	inline double signum(double value) const {
	...
	Interval3D refract(const Interval3D &wi, const Interval3D  &n, double eta)  const {
	...
	bool isValid(int *sign, double *etaInside) const
	{
		...
		for (int i = 1; i < getLength() - 1; i++)
		{
			...
			if (nP2Pre.isEmpty()) return false;
	...
	double getTriangleProduct() const
	{
		double ans = 1;
		for (int i = 0; i < m_countBounce; i++)
		{
